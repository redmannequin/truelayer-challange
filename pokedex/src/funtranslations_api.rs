//! containes funtranslations api types and functions
...
const FUNTRANSLATION_YODA_ENDPOINT: &str = "https://api.funtranslations.com/translate/yoda.json";
const FUNTRANSLATION_SHAKESPEARE_ENDPOINT: &str =
...
pub async fn translate_to_yoda(text: &str) -> Result<String> {
    let yoda_url = format!("{}?text={}", FUNTRANSLATION_YODA_ENDPOINT, text);
...
pub async fn translate_to_shakespeare(text: &str) -> Result<String> {
    let shakespeare_url = format!("{}?text={}", FUNTRANSLATION_SHAKESPEARE_ENDPOINT, text);
